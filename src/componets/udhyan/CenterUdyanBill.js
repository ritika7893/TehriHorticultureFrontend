...
const readJsonResponse = async (response) => {
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    ...
    throw new Error(
      `सर्वर ने JSON के बजाय HTML/अन्य response भेजा। HTTP ${response.status}${preview ? ` — ${preview}` : ""}`,
    );
  }
  try {
...
export default function CenterUdyanBill() {
  ...
  const loadStandards = async () => {
    ...
    try {
      const response = await apiFetch(
        `${API_BASE}/crop-standards/?financial_year=${encodeURIComponent(financialYear)}`,
      );
  ...
  const loadUploadedBills = async () => {
    ...
    try {
      const centerParam = authCenterName
        ? `?center=${encodeURIComponent(authCenterName)}`
        : "";
  ...
  const viewUploadedBill = (savedBill) => {
    ...
    setMessage(
      `बिल #${savedBill.id} देखा जा रहा है (आप इसे संशोधित कर प्रिंट कर सकते हैं, पर सेव नहीं कर सकते)।`,
    );

    if (!matchingStandard) {
      setMessage(
        `बिल #${savedBill.id} खोला गया है। संबंधित मानक उपलब्ध नहीं है।`,
      );
    }
  };
      ...
      <div className="control-panel">
        <div className="control-heading">
          <div>
            ...
            <p>
              केवल दर्शन मोड — बिल देखा एवं प्रिंट किया जा सकता है
              (संपादन/हटाना/जोड़ना अनुमत नहीं)
            </p>
          </div>
