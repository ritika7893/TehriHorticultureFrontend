...
const CenterSelect = ({ bill, onChange, centers, loading, className = "" }) => {
  const selectedCenter = centers.find(
    (center) => String(center.name) === String(bill.center ?? ""),
  );
...
const CenterAutoFill = ({ bill, className = "" }) => {
  return (
    <input
      className={`doc-input ${className}`}
      value={bill.center ?? ""}
...
export default function UdyanBill() {
  ...
  const loadCenters = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail || data?.error || "केंद्र सूची प्राप्त नहीं हो सकी।",
        );
      }
      ...
      const activeList = list.filter((center) => center.is_active !== false);

      setCenters(activeList);
    } catch (error) {
      console.error("loadCenters:", error);

      setMessage(error.message || "केंद्र सूची प्राप्त नहीं हो सकी।");
    } finally {
      ...
      <div className="document-area" id="printDocument">
        ...
        <div className="a4-page">
          ...
          <div className="officer-sign">
            ...
            उद्यान सचल दल
            <br />
            केन्द्र <CenterAutoFill bill={bill} className="officer-input" />
          </div>
        </div>
