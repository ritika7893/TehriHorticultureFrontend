...
const normalizeBillingRow = (item) => ({
  ...
  vahan:
    (item.anudan_name ?? item.vahan) == null
      ? ""
      : String(item.anudan_name ?? item.vahan).trim(),
  anudan_name:
    (item.anudan_name ?? item.vahan) == null
      ? ""
      : String(item.anudan_name ?? item.vahan).trim(),
              ...
              groupedRows.map(([name, group], index) => (
                <tr key={`summary-mad-upmad-${summaryWiseView}-${name}`}>
                    ...
                    if (summaryWiseShowMatra) {
                      cells.push(
                        <td key={`yw-matra-${name}-${plan}`}>
                          {group.physical[plan]
                            ? summaryWiseFormatExactNumber(group.physical[plan])
                            : ""}
                        </td>,
                      );
                    }
                    if (summaryWiseShowAnudan) {
                      cells.push(
                        <td key={`yw-anudan-${name}-${plan}`}>
                          {group.financial[plan]
                            ? summaryWiseFmtR(group.financial[plan])
                            : ""}
                        </td>,
                      );
                    }
                  ...
                  <td className="tot">
                    {summaryWiseFormatExactNumber(
                      selectedPlans.reduce(
                        (sum, plan) => sum + (group.physical[plan] || 0),
                        0,
                      ),
                    )}
                  </td>
                  <td className="tot">
                    {summaryWiseFmtR(
                      selectedPlans.reduce(
                        (sum, plan) => sum + (group.financial[plan] || 0),
                        0,
                      ),
                    )}
                  </td>
                  ...
                  if (summaryWiseShowMatra) {
                    cells.push(
                      <td key={`yw-total-matra-${plan}`} className="tot">
                        {physicalTotal
                          ? summaryWiseFormatExactNumber(physicalTotal)
                          : ""}
                      </td>,
                    );
                  }
                  ...
                  {summaryWiseFormatExactNumber(
                    groupedRows.reduce(
                      (sum, [, group]) =>
                        sum +
                        selectedPlans.reduce(
                          (inner, plan) => inner + (group.physical[plan] || 0),
                          0,
                        ),
                      0,
                    ),
                  ...
                  {summaryWiseFmtR(
                    groupedRows.reduce(
                      (sum, [, group]) =>
                        sum +
                        selectedPlans.reduce(
                          (inner, plan) => inner + (group.financial[plan] || 0),
                          0,
                        ),
                      0,
                    ),
...
const Dashboard = () => {
  ...
  const reportApiData = useMemo(
    () => billingData.map(normalizeBillingRow),
    [billingData],
  );
  ...
  const [selectedSubInvestmentSchemes, setSelectedSubInvestmentSchemes] =
    useState([]);

  // State for investment table filters
  const [selectedMainInvestments, setSelectedMainInvestments] = useState([]);
  const [selectedMainInvestmentSchemes, setSelectedMainInvestmentSchemes] =
    useState([]);
  ...
  const schemeOptions = useMemo(() => {
    return [
      ...new Set(
        dateFilteredData.map((item) => item.scheme_name).filter(Boolean),
      ),
    ].sort((a, b) => String(a).localeCompare(String(b), "hi"));
  }, [dateFilteredData]);
  ...
  const filteredData = useMemo(() => {
    ...
    return dateFilteredData.filter((item) => {
      ...
      const investmentMatch =
        ...
        investmentValues.includes(item.upnivesh);
  ...
  const centerChartData = useMemo(() => {
    ...
    const sortedEntries = Object.entries(centerData).sort(
  ...
  const vidhanChartData = useMemo(() => {
    ...
    const sortedEntries = Object.entries(vidhanData).sort(
      (a, b) => b[1].total - a[1].total,
    );
    const fullLabels = sortedEntries.map(([key]) => key);
    const truncatedLabels = fullLabels.map((l) =>
      l.length > 20 ? l.substring(0, 18) + "..." : l,
    );
    ...
    return {
      doughnut: {
        ...
        datasets: [
          {
            data: limitedFull.map(
              (l) => (vidhanData[l] && vidhanData[l].total) || 0,
            ),
            backgroundColor: colors.slice(0, limitedFull.length),
            borderColor: colors
              .slice(0, limitedFull.length)
  ...
  useEffect(() => {
    ...
    return () => {
      cards.forEach((card) => {
        const header = card.querySelector(".chart-card-header");
        if (header && header.__toggle)
          header.removeEventListener("click", header.__toggle);
      });
    };
  }, [filteredData]);
  ...
  const investmentSubsidyChartData = useMemo(() => {
    ...
    const labels = Object.keys(investmentData).sort(
      (a, b) => investmentData[b].subsidy - investmentData[a].subsidy,
    );
  ...
  const vidhanCombinedTableData = useMemo(() => {
    ...
    filteredData.forEach((item) => {
      ...
      if (!vidhanSchemeData[vidhan][scheme])
        vidhanSchemeData[vidhan][scheme] = {
          subsidy: 0,
          farmerShare: 0,
          total: 0,
          quantity: 0,
    ...
    const allSchemes = new Set();
    vidhans.forEach((v) =>
      Object.keys(vidhanSchemeData[v]).forEach((s) => allSchemes.add(s)),
    );
    const schemes = Array.from(allSchemes).sort();
    ...
    const quantities = {};
    vidhans.forEach((v) => {
      totals[v] = schemes.reduce(
        (sum, sch) =>
          sum +
          ((vidhanSchemeData[v][sch] && vidhanSchemeData[v][sch].subsidy) || 0),
        0,
      );
  ...
  const centerCombinedTableData = useMemo(() => {
    ...
    filteredData.forEach((item) => {
      ...
      if (!centerSchemeData[center][scheme])
        centerSchemeData[center][scheme] = {
          subsidy: 0,
          farmerShare: 0,
          total: 0,
          quantity: 0,
    ...
    const allSchemes = new Set();
    centers.forEach((c) =>
      Object.keys(centerSchemeData[c]).forEach((s) => allSchemes.add(s)),
    );
    const schemes = Array.from(allSchemes).sort();
    ...
    const quantities = {};
    centers.forEach((c) => {
      totals[c] = schemes.reduce(
        (sum, sch) =>
          sum +
          ((centerSchemeData[c][sch] && centerSchemeData[c][sch].subsidy) || 0),
        0,
      );
  ...
  const combinedChartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [
        {
          data: combinedTableData.investments
            .slice(0, 10)
            .map((inv) => combinedTableData.totals[inv]),
          backgroundColor: colors.slice(
            0,
            combinedTableData.investments.length,
          ),
          borderColor: colors
            .slice(0, combinedTableData.investments.length)
            .map((c) => c.replace("0.8", "1")),
          borderWidth: 2,
        },
      ],
  ...
  const getFilterStatusText = () => {
    ...
    if (isDateFilterApplied && (appliedStartDate || appliedEndDate)) {
      filters.push(
        `तिथि: ${formatDate(appliedStartDate) || "N/A"} से ${formatDate(appliedEndDate) || "N/A"}`,
      );
    }
    if (selectedSchemes.length > 0) {
      filters.push(
        `क्रय क्रय योजना: ${selectedSchemes.map((s) => s.label).join(", ")}`,
      );
    }
    if (selectedInvestments.length > 0) {
      filters.push(
        `उप-मद: ${selectedInvestments.map((i) => i.label).join(", ")}`,
      );
    }
    return filters.length > 0
      ? filters.join(" | ")
      : "सभी डेटा (कोई फ़िल्टर नहीं)";
  };
  ...
  const getReportData = () => {
    ...
    filteredData.forEach((item) => {
      ...
      investmentWise[investment].quantity +=
        parseFloat(item.allocated_quantity) || 0;
      investmentWise[investment].farmerShare +=
        parseFloat(item.amount_of_farmer_share) || 0;
      investmentWise[investment].subsidy +=
        parseFloat(item.amount_of_subsidy) || 0;
  ...
  const getMainInvestmentSubsidyData = () => {
    ...
    filteredData.forEach((item) => {
      ...
      mainInvestmentSubsidy[investment].subsidy +=
        parseFloat(item.amount_of_subsidy) || 0;
  ...
  const getCombinedData = () => {
    ...
    filteredData.forEach((item) => {
      ...
      if (!investmentSchemeData[investment])
        investmentSchemeData[investment] = {};
      if (!investmentSchemeData[investment][scheme])
        investmentSchemeData[investment][scheme] = 0;
  ...
  const getSubCombinedData = () => {
    ...
    filteredData.forEach((item) => {
      ...
      if (!subInvestmentSchemeData[subInvestment])
        subInvestmentSchemeData[subInvestment] = {};
      if (!subInvestmentSchemeData[subInvestment][scheme])
        subInvestmentSchemeData[subInvestment][scheme] = 0;
    ...
    subInvestments.forEach((subInv) => {
      totals[subInv] = schemes.reduce(
        (sum, sch) => sum + (data[subInv][sch] || 0),
        0,
      );
    });
    const grandTotal = subInvestments.reduce(
      (sum, subInv) => sum + totals[subInv],
      0,
    );
  ...
  const generatePDF = (action = "download") => {
    ...
    const schemeData =
      schemeChartData && schemeChartData.rawData ? schemeChartData.rawData : {};
    const investmentData =
      investmentChartData && investmentChartData.rawData
        ? investmentChartData.rawData
        : {};
    const combined = combinedTableData || {
      investments: [],
      schemes: [],
      data: {},
      totals: {},
    ...
    const subCombined = subCombinedTableData || {
      subInvestments: [],
      schemes: [],
      data: {},
      totals: {},
    ...
    const centerCombined = centerCombinedTableData || {
      centers: [],
      schemes: [],
      data: {},
      totals: {},
    ...
    const vidhanCombined = vidhanCombinedTableData || {
      vidhans: [],
      schemes: [],
      data: {},
      totals: {},
      ...
      <div style="font-family: 'Noto Sans Devanagari', 'Mangal', Arial, sans-serif; padding: 20px; color: #333;">
           ...
           <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
             ...
             <tbody>
               ${Object.entries(schemeData)
                 ...
                 .sort(
                   (a, b) =>
                     (b[1][selectedRashi] || 0) - (a[1][selectedRashi] || 0),
                 )
                 .map(
                   ([name, val], idx) => `
             ...
             <tfoot>
               <tr style="font-weight:700; background:#f1f5f9;">
                 ...
                 <td style="border:1px solid #ddd; padding:6px; text-align:right;">${Object.entries(
                   schemeData || {},
                 )
                   .filter(
                     ([label]) =>
                   ...
                   .reduce((s, [_, v]) => s + ((v && v.quantity) || 0), 0)
                   .toFixed(2)}</td>
                 <td style="border:1px solid #ddd; padding:6px; text-align:right;">₹${Object.entries(
                   schemeData || {},
                 )
                   ...
                   .reduce((s, [_, v]) => s + ((v && v[selectedRashi]) || 0), 0)
                   .toLocaleString("hi-IN", { minimumFractionDigits: 2 })}</td>
               </tr>
             </tfoot>
           </table>
         </div>
        ...
        <div style="margin-bottom: 20px; page-break-inside: avoid;">
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
            ...
            <tfoot>
              <tr style="font-weight:700; background:#f1f5f9;">
                <td colSpan="2" style="border:1px solid #ddd; padding:6px;">कुल</td>
                <td style="border:1px solid #ddd; padding:6px; text-align:right;">${Object.values(
                  investmentData || {},
                )
                  .reduce((s, v) => s + ((v && v.quantity) || 0), 0)
                  .toFixed(2)}</td>
                <td style="border:1px solid #ddd; padding:6px; text-align:right;">₹${Object.values(
                  investmentData || {},
                )
                  .reduce((s, v) => s + ((v && v[selectedRashi]) || 0), 0)
                  .toLocaleString("hi-IN", { minimumFractionDigits: 2 })}</td>
              </tr>
            </tfoot>
          </table>
        ...
        `
         ...
         <div style="margin-bottom: 20px; page-break-inside: avoid;">
           ...
           <table style="width: 100%; border-collapse: collapse; font-size: 9px;">
             <thead>
               <tr style=" color: #000;">
                 ...
                 ${subCombined.schemes
                   .filter(
                     (s) =>
                       selectedSubInvestmentSchemes.length === 0 ||
                       selectedSubInvestmentSchemes.some(
                         (selected) => selected.value === s,
                       ),
                   )
                   .map(
                     (s) =>
                       `<th style="border:1px solid #ddd; padding:6px; text-align:center;" colspan="2">${s}</th>`,
                   )
                   .join("")}
               ...
               <tr style="background:#e9eef8;">
                 ...
                 ${subCombined.schemes
                   ...
                   .map(
                     () =>
                       `<th style="border:1px solid #ddd; padding:6px; text-align:right;">मात्रा</th><th style="border:1px solid #ddd; padding:6px; text-align:right;">${rashiOptions.find((opt) => opt.value === selectedRashi)?.label}</th>`,
                   )
                   .join("")}
             ...
             <tbody>
               ${subCombined.subInvestments
                 .filter(
                   (subInv) =>
                     selectedSubInvestments.length === 0 ||
                     selectedSubInvestments.some(
                       (selected) => selected.value === subInv,
                     ),
                 )
                 .map(
                   (subInv, idx) => `
                 <tr style="background: ${idx % 2 === 0 ? "#ffffff" : "#f8f9fa"};">
                   ...
                   `,
                     ...
                     .join("")}
                   <td style="border:1px solid #ddd; padding:6px; text-align:right;">${subCombined.schemes
                     .filter(
                       (s) =>
                         selectedSubInvestmentSchemes.length === 0 ||
                         selectedSubInvestmentSchemes.some(
                     ...
                     .reduce(
                       (sum, s) =>
                         sum +
                         ((subCombined.data[subInv] &&
                           subCombined.data[subInv][s] &&
                           subCombined.data[subInv][s].quantity) ||
                           0),
                       0,
                     )
                     .toFixed(2)}</td>
                   <td style="border:1px solid #ddd; padding:6px; text-align:right;">₹${subCombined.schemes
                     ...
                     .reduce(
                       (sum, s) =>
                         ...
                         ((subCombined.data[subInv] &&
                           subCombined.data[subInv][s] &&
                           subCombined.data[subInv][s][selectedRashi]) ||
                           0),
                       0,
                     )
             ...
             <tfoot>
               <tr style="font-weight:700; background:#f1f5f9;">
                 ...
                 ${subCombined.schemes
                   ...
                   .map(
                     (s) => `
                   <td style="border:1px solid #ddd; padding:6px; text-align:right;">${subCombined.subInvestments
                     .filter(
                       (subInv) =>
                         selectedSubInvestments.length === 0 ||
                         selectedSubInvestments.some(
                     ...
                     .reduce(
                       (sum, si) =>
                         sum +
                         ((subCombined.data[si] &&
                           subCombined.data[si][s] &&
                           subCombined.data[si][s].quantity) ||
                           0),
                       0,
                     )
                     .toFixed(2)}</td>
                   <td style="border:1px solid #ddd; padding:6px; text-align:right;">₹${subCombined.subInvestments
                     ...
                     .reduce(
                       (sum, si) =>
                         ...
                         ((subCombined.data[si] &&
                           subCombined.data[si][s] &&
                           subCombined.data[si][s][selectedRashi]) ||
                           0),
                       0,
                     )
                     .toLocaleString("hi-IN", {
        ...
        <div style="margin-bottom: 20px; page-break-inside: avoid;">
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
            ...
            <tbody>
              ${Object.entries(centerChartData.rawData)
                .sort(
                  (a, b) =>
                    (b[1][selectedRashi] || 0) - (a[1][selectedRashi] || 0),
                )
           ...
           <table style="width: 100%; border-collapse: collapse; font-size: 8px; table-layout: fixed;">
             <thead>
               <tr style=" color:#000;">
                 ...
                 ${centerCombined.schemes
                   .filter(
                     (s) =>
                       selectedCenterSchemes.length === 0 ||
                       selectedCenterSchemes.some(
                         (selected) => selected.value === s,
                       ),
                   )
                   .map(
                     (s) =>
                       `<th style="border:1px solid #ddd; padding:5px; text-align:center;" colspan="2">${s.substring(0, 18)}${s.length > 18 ? ".." : ""}</th>`,
                   )
                   .join("")}
             ...
             <tbody>
               ${centerCombined.centers
                 .filter(
                   (c) =>
                     selectedCenters.length === 0 ||
                     selectedCenters.some((selected) => selected.value === c),
                 )
                 .map(
                   (c, idx) => `
                 <tr style="background: ${idx % 2 === 0 ? "#ffffff" : "#f8f9fa"};">
                   ...
                   `,
                     ...
                     .join("")}
                   <td style="border:1px solid #ddd; padding:5px; text-align:right;">${centerCombined.schemes
                     .filter(
                       (s) =>
                         selectedCenterSchemes.length === 0 ||
                         selectedCenterSchemes.some(
                     ...
                     .reduce(
                       (sum, s) =>
                         sum +
                         ((centerCombined.data[c] &&
                           centerCombined.data[c][s] &&
                           centerCombined.data[c][s].quantity) ||
                           0),
                       0,
                     )
                     .toFixed(2)}</td>
                   <td style="border:1px solid #ddd; padding:5px; text-align:right;">₹${centerCombined.schemes
                     ...
                     .reduce(
                       (sum, s) =>
                         ...
                         ((centerCombined.data[c] &&
                           centerCombined.data[c][s] &&
                           centerCombined.data[c][s][selectedRashi]) ||
                           0),
                       0,
                     )
                     .toLocaleString("hi-IN", {
             ...
             <tfoot>
               <tr style="font-weight:700; background:#f1f5f9;">
                 ...
                 ${centerCombined.schemes
                   ...
                   .map(
                     (s) => `
                   <td style="border:1px solid #ddd; padding:5px; text-align:right;">${centerCombined.centers
                     .filter(
                       (c) =>
                         selectedCenters.length === 0 ||
                         selectedCenters.some(
        ...
        <div style="margin-bottom: 20px; page-break-inside: avoid;">
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
            ...
            <tbody>
              ${Object.entries(vidhanChartData.rawData)
                .sort(
                  (a, b) =>
                    (b[1][selectedRashi] || 0) - (a[1][selectedRashi] || 0),
                )
           ...
           <table style="width: 100%; border-collapse: collapse; font-size: 8px; table-layout: fixed;">
             <thead>
               <tr style=" color:#000;">
                 ...
                 ${vidhanCombined.schemes
                   .filter(
                     (s) =>
                       selectedVidhanSchemes.length === 0 ||
                       selectedVidhanSchemes.some(
                         (selected) => selected.value === s,
                       ),
                   )
                   .map(
             ...
             <tbody>
               ${vidhanCombined.vidhans
                 .filter(
                   (v) =>
                     selectedVidhanSabhas.length === 0 ||
                     selectedVidhanSabhas.some(
                 ...
                 <tr style="background: ${idx % 2 === 0 ? "#ffffff" : "#f8f9fa"};">
                   ...
                   `,
                     ...
                     .join("")}
                   <td style="border:1px solid #ddd; padding:5px; text-align:right;">${vidhanCombined.schemes
                     .filter(
                       (s) =>
                         selectedVidhanSchemes.length === 0 ||
                         selectedVidhanSchemes.some(
                     ...
                     .reduce(
                       (sum, s) =>
                         sum +
                         ((vidhanCombined.data[v] &&
                           vidhanCombined.data[v][s] &&
                           vidhanCombined.data[v][s].quantity) ||
                           0),
                       0,
                     )
                     .toFixed(2)}</td>
                   <td style="border:1px solid #ddd; padding:5px; text-align:right;">₹${vidhanCombined.schemes
                     ...
                     .reduce(
                       (sum, s) =>
                         ...
                         ((vidhanCombined.data[v] &&
                           vidhanCombined.data[v][s] &&
                           vidhanCombined.data[v][s][selectedRashi]) ||
                           0),
                       0,
                     )
                     .toLocaleString("hi-IN", {
             ...
             <tfoot>
               <tr style="font-weight:700; background:#f1f5f9;">
                 ...
                 ${vidhanCombined.schemes
                   ...
                   .map(
                     (s) => `
                   <td style="border:1px solid #ddd; padding:5px; text-align:right;">${vidhanCombined.vidhans
                     .filter(
                       (v) =>
                         selectedVidhanSabhas.length === 0 ||
                         selectedVidhanSabhas.some(
           ...
           <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
             ...
             <tfoot>
               <tr style="font-weight: 700; background-color: #f1f5f9;">
                 ...
                 <td style="border: 1px solid #ddd; padding: 6px; text-align: right; font-weight: bold;">${Object.entries(
                   investmentChartData.rawData || {},
                 )
                   .filter(
                     ([name]) =>
            ...
            <table style="width: 100%; border-collapse: collapse; font-size: 9px;">
              <thead>
                <tr style="background: #ffffff; color: black;">
                  ...
                  ${combined.schemes
                    .filter(
                      (s) =>
                        selectedMainInvestmentSchemes.length === 0 ||
                        selectedMainInvestmentSchemes.some(
                          (selected) => selected.value === s,
                        ),
                    )
                    .map(
                      (s) =>
                        `<th style="border:1px solid #ddd; padding:6px; color: black; text-align:center;" colspan="2">${s}</th>`,
                    )
                    .join("")}
                ...
                <tr style="background:#ffffff; color: black;">
                  ...
                  ${combined.schemes
                    ...
                    .map(
                      () =>
                        `<th style="border:1px solid #ddd; padding:6px; color: black; text-align:right;">मात्रा</th><th style="border:1px solid #ddd; padding:6px; color: black; text-align:right;">${rashiOptions.find((opt) => opt.value === selectedRashi)?.label}</th>`,
                    )
                    .join("")}
              ...
              <tbody>
                ${combined.investments
                  .filter(
                    (inv) =>
                      selectedMainInvestments.length === 0 ||
                      selectedMainInvestments.some(
                        (selected) => selected.value === inv,
                      ),
                  )
                  .map(
                    (inv, idx) => `
                  <tr style="background: ${idx % 2 === 0 ? "#ffffff" : "#f8f9fa"};">
                    ...
                    `,
                      ...
                      .join("")}
                    <td style="border:1px solid #ddd; padding:6px; text-align:right;">${combined.schemes
                      .filter(
                        (s) =>
                          selectedMainInvestmentSchemes.length === 0 ||
                          selectedMainInvestmentSchemes.some(
                    ...
                    <td style="border:1px solid #ddd; padding:6px; text-align:right;">₹${combined.schemes
                      ...
                      .reduce(
                        (sum, s) =>
                          ...
                          ((combined.data[inv] &&
                            combined.data[inv][s] &&
                            combined.data[inv][s][selectedRashi]) ||
                            0),
                        0,
                      )
                      .toLocaleString("hi-IN", {
              ...
              <tfoot>
                <tr style="background: #f8f9fa; color: #000000;">
                  ...
                  ${combined.schemes
                    ...
                    .map(
                      (s) => `
                    <td style="border:1px solid #ddd; padding:6px; text-align:right;">${combined.investments
                      .filter(
                        (inv) =>
                          selectedMainInvestments.length === 0 ||
                          selectedMainInvestments.some(
  ...
  const handleShare = async (type) => {
    ...
    try {
      if (type === "pdf") {
        ...
        const { investments, schemes, data, totals, grandTotal } =
          getCombinedData();
        const {
          subInvestments,
          schemes: subSchemes,
        ...
        if (openCollapses.includes("scheme")) {
          ...
          const schemeHeaders = [
            ...
            "योजना",
            "भौतिक पूर्ति ",
            rashiColumnLabelExcel[selectedRashi],
          ];
          const schemeRows = Object.entries(schemeRaw)
            ...
            .sort(
              (a, b) => (b[1][selectedRashi] || 0) - (a[1][selectedRashi] || 0),
            )
            .map(([name, val], idx) => [
              idx + 1,
              name,
        ...
        if (openCollapses.includes("subInvestment")) {
          ...
          const rows = filteredSubInvestments.map((si, idx) => [
            ...
            filteredSchemes
              .reduce(
                (sum, s) => sum + Number(sub.data?.[si]?.[s]?.quantity || 0),
                0,
              )
              .toFixed(2),
            filteredSchemes
              .reduce(
                (sum, s) =>
                  sum + Number(sub.data?.[si]?.[s]?.[selectedRashi] || 0),
                0,
              )
              .toFixed(2),
          ]);
        ...
        if (openCollapses.includes("mainInvestment")) {
          ...
          rows.push([
            ...
            Object.values(raw)
              ...
              .toFixed(2),
            Object.values(raw)
              .reduce((s, v) => s + Number(v?.[selectedRashi] || 0), 0)
              .toFixed(2),
          ]);
        ...
        if (openCollapses.includes("centerCombined")) {
          ...
          const rows = filteredCenters.map((c, idx) => [
            ...
            filteredSchemes
              .reduce(
                (sum, s) => sum + Number(center.data?.[c]?.[s]?.quantity || 0),
                0,
              )
              .toFixed(2),
            filteredSchemes
              .reduce(
                (sum, s) =>
                  sum + Number(center.data?.[c]?.[s]?.[selectedRashi] || 0),
                0,
              )
              .toFixed(2),
          ]);
        ...
        if (openCollapses.includes("vidhanCombined")) {
          ...
          const rows = filteredVidhans.map((v, idx) => [
            ...
            filteredSchemes
              .reduce(
                (sum, s) => sum + Number(vidhan.data?.[v]?.[s]?.quantity || 0),
                0,
              )
              .toFixed(2),
            filteredSchemes
              .reduce(
                (sum, s) =>
                  sum + Number(vidhan.data?.[v]?.[s]?.[selectedRashi] || 0),
                0,
              )
              .toFixed(2),
          ]);
        ...
        if (openCollapses.includes("investmentCombined")) {
          ...
          const rows = filteredInvestments.map((inv, idx) => [
            ...
            filteredSchemes
              .reduce(
                (sum, s) => sum + Number(comb.data?.[inv]?.[s]?.quantity || 0),
                0,
              )
              .toFixed(2),
            filteredSchemes
              .reduce(
                (sum, s) =>
                  sum + Number(comb.data?.[inv]?.[s]?.[selectedRashi] || 0),
                0,
              )
              .toFixed(2),
          ]);
      ...
      setShareFileName("");
      alert(
        `फाइल शेयर करने के लिए बनाने में त्रुटि हुई: ${err?.message || "Unknown error"}`,
      );
    }
  };
  ...
  const handleDirectShare = async () => {
    ...
    try {
      ...
      handleShareDownload();
      alert(
        "इस ब्राउज़र में direct file sharing उपलब्ध नहीं है। फाइल डाउनलोड हो गई है; इसे WhatsApp/Email आदि में अटैच करके शेयर करें।",
      );
    } catch (err) {
  ...
  const generateExcel = (action = "download") => {
    ...
    if (openCollapses.includes("scheme")) {
      const schemeData =
        schemeChartData && schemeChartData.rawData
    ...
    if (openCollapses.includes("investment")) {
      ...
      const investmentWs = XLSX.utils.aoa_to_sheet([
        investmentHeaders,
        ...investmentRows,
      ]);
      investmentWs["!cols"] = [
    ...
    if (openCollapses.includes("subInvestment")) {
      ...
      const filteredSchemes = sub.schemes.filter(
        (scheme) =>
          selectedSubInvestmentSchemes.length === 0 ||
          selectedSubInvestmentSchemes.some((s) => s.value === scheme),
      );
      const filteredSubInvestments = sub.subInvestments.filter(
        (subInv) =>
          selectedSubInvestments.length === 0 ||
          selectedSubInvestments.some((s) => s.value === subInv),
      );
      const subHeaders = [
        "#",
        "उप-मद",
      ...
      const subRows = filteredSubInvestments.map((si, idx) => [
        ...
        ...filteredSchemes.flatMap((s) => [
          (
            (sub.data[si] && sub.data[si][s] && sub.data[si][s].quantity) ||
            0
          ).toFixed(2),
          (
            (sub.data[si] &&
              sub.data[si][s] &&
              sub.data[si][s][selectedRashi]) ||
            0
          ).toFixed(2),
        ]),
        filteredSchemes
      ...
      subRows.push([
        ...
        "",
        ...filteredSchemes.flatMap((s) => [
          filteredSubInvestments
            .reduce(
              (sum, si) =>
                sum +
                ((sub.data[si] &&
                  sub.data[si][s] &&
                  sub.data[si][s].quantity) ||
                  0),
              0,
            )
            .toFixed(2),
      ...
      subCombinedWs["!cols"] = [
        { wch: 6 },
        { wch: 30 },
        ...filteredSchemes.flatMap(() => [{ wch: 12 }, { wch: 15 }]),
        { wch: 15 },
        { wch: 18 },
      ];
    ...
    if (openCollapses.includes("mainInvestment")) {
      ...
      mainInvestmentRows.push([
        "कुल",
        "",
        Object.values(investmentChartData.rawData || {})
          .reduce((s, v) => s + ((v && v.quantity) || 0), 0)
          .toFixed(2),
        Object.values(investmentChartData.rawData || {})
          .reduce((s, v) => s + ((v && v[selectedRashi]) || 0), 0)
      ...
      const mainInvestmentWs = XLSX.utils.aoa_to_sheet([
        mainInvestmentHeaders,
        ...mainInvestmentRows,
      ]);
      mainInvestmentWs["!cols"] = [
        { wch: 6 },
        { wch: 40 },
        { wch: 15 },
        { wch: 18 },
    ...
    if (openCollapses.includes("centerCombined")) {
      ...
      const filteredSchemes = center.schemes.filter(
        (scheme) =>
          selectedCenterSchemes.length === 0 ||
          selectedCenterSchemes.some((s) => s.value === scheme),
      );
      const filteredCenters = center.centers.filter(
        (center) =>
          selectedCenters.length === 0 ||
          selectedCenters.some((c) => c.value === center),
      );
      const centerHeaders = [
        "#",
        "केंद्र",
      ...
      const centerRows = filteredCenters.map((c, idx) => [
        ...
        ...filteredSchemes.flatMap((s) => [
          (
            (center.data[c] &&
              center.data[c][s] &&
              center.data[c][s].quantity) ||
            0
          ).toFixed(2),
          (
            (center.data[c] &&
              center.data[c][s] &&
              center.data[c][s][selectedRashi]) ||
            0
          ).toFixed(2),
        ]),
        filteredSchemes
    ...
    if (openCollapses.includes("vidhanCombined")) {
      ...
      const filteredSchemes = vidhan.schemes.filter(
        (scheme) =>
          selectedVidhanSchemes.length === 0 ||
          selectedVidhanSchemes.some((s) => s.value === scheme),
      );
      const filteredVidhans = vidhan.vidhans.filter(
        (vidhan) =>
          selectedVidhanSabhas.length === 0 ||
          selectedVidhanSabhas.some((v) => v.value === vidhan),
      );
      const vidhanHeaders = [
        "#",
        "विधानसभा",
      ...
      const vidhanRows = filteredVidhans.map((v, idx) => [
        ...
        ...filteredSchemes.flatMap((s) => [
          (
            (vidhan.data[v] &&
              vidhan.data[v][s] &&
              vidhan.data[v][s].quantity) ||
            0
          ).toFixed(2),
          (
            (vidhan.data[v] &&
              vidhan.data[v][s] &&
              vidhan.data[v][s][selectedRashi]) ||
            0
          ).toFixed(2),
        ]),
        filteredSchemes
    ...
    if (openCollapses.includes("investmentCombined")) {
      ...
      const filteredSchemes = comb.schemes.filter(
        (scheme) =>
          selectedMainInvestmentSchemes.length === 0 ||
          selectedMainInvestmentSchemes.some((s) => s.value === scheme),
      );
      const filteredInvestments = comb.investments.filter(
        (inv) =>
          selectedMainInvestments.length === 0 ||
          selectedMainInvestments.some((s) => s.value === inv),
      );
      ...
      const combinedRows = filteredInvestments.map((inv, idx) => [
        ...
        ...filteredSchemes.flatMap((s) => [
          (
            (comb.data[inv] &&
              comb.data[inv][s] &&
              comb.data[inv][s].quantity) ||
            0
          ).toFixed(2),
          (
            (comb.data[inv] &&
              comb.data[inv][s] &&
              comb.data[inv][s][selectedRashi]) ||
            0
          ).toFixed(2),
        ]),
        filteredSchemes
      ...
      const combinedWs = XLSX.utils.aoa_to_sheet([
        combinedHeaders,
        ...combinedRows,
      ]);
      combinedWs["!cols"] = [
    ...
    if (action === "download") {
      ...
      wb.SheetNames.forEach((sheetName) => {
        sheetData[sheetName] = XLSX.utils.sheet_to_json(wb.Sheets[sheetName], {
          header: 1,
        });
      });
      setExcelPreviewData({
    ...
    <>
                      ...
                      <div className="d-flex gap-2 justify-content-md-end flex-wrap">
                        ...
                        <Dropdown as={ButtonGroup} size="sm">
                          <Button
                            variant="success"
                            size="sm"
                            onClick={() => generateExcel("download")}
                          >
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            एक्सेल प्रिव्यू - {excelPreviewData?.filename}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body style={{ maxHeight: "500px", overflowY: "auto" }}>
          {excelPreviewData && (
            <div>
              {Object.entries(excelPreviewData.sheets).map(
                ([sheetName, rows]) => (
                  <div key={sheetName} className="mb-4">
                    ...
                    {rows.length > 50 && (
                      ...
                      >
                        ... और भी {rows.length - 50} पंक्तियाँ हैं। डाउनलोड करने
                        के लिए "डाउनलोड" बटन का उपयोग करें।
                      </div>
                    )}
                  </div>
                ),
