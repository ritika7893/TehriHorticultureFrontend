...
const mainTableColumnMapping = {
  ...
  reportDate: {
    header: "रिपोर्ट तारीख",
    accessor: (item) =>
      item.billing_date
        ? new Date(item.billing_date).toLocaleDateString("hi-IN")
        : "N/A",
  },
  status: {
    header: "स्थिति",
  ...
  totalItems: {
    header: "कुल आइटम",
    accessor: (item) => (item.component_data ? item.component_data.length : 0),
  },
...
const componentColumnMapping = {
  ...
  total_amount: {
    header: "कुल राशि",
    accessor: (item) => `₹${item.total_amount?.toFixed(2) || "0.00"}`,
  },
};
...
const MPR = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let originalData =
        originalYearlyData.length > 0
          ? originalYearlyData
          : (await axios.get(YEARLY_DATA_URL)).data;
      ...
      if (filters.year) {
        filteredYearlyData = filteredYearlyData.filter(
          (item) =>
            item.billing_date &&
            new Date(item.billing_date).getFullYear() === filters.year,
        );
      }
      ...
      if (filters.month) {
        filteredYearlyData = filteredYearlyData.filter(
          (item) =>
            item.billing_date &&
            new Date(item.billing_date).getMonth() + 1 === filters.month,
        );
      }
      ...
      if (filters.source_of_receipt && filters.source_of_receipt.length > 0) {
        ...
        filteredYearlyData = filteredYearlyData.filter((item) =>
          selectedSources.includes(item.source_of_receipt),
        );
      }
      ...
      if (filters.selected_dates) {
        const selectedDates = filters.selected_dates
          .split(",")
          .map((d) => d.trim());
        filteredYearlyData = filteredYearlyData.filter((item) => {
          if (!item.billing_date) return false;
          const itemDateStr = new Date(item.billing_date)
            .toISOString()
            .split("T")[0];
      ...
      if (filters.investment_name && filters.investment_name.length > 0) {
        const selectedInvestments = filters.investment_name.map((i) => i.value);
        filteredMonthlyData = filteredMonthlyData.filter(
          (item) =>
            item.component_data &&
            item.component_data.some((comp) =>
              selectedInvestments.includes(comp.investment_name),
            ),
        );
      }
      if (filters.scheme_name && filters.scheme_name.length > 0) {
        ...
        filteredMonthlyData = filteredMonthlyData.filter(
          (item) =>
            item.component_data &&
            item.component_data.some((comp) =>
              selectedSchemes.includes(comp.scheme_name),
            ),
        );
      }
      if (filters.from_date && filters.to_date) {
      ...
      filteredMonthlyData = filteredMonthlyData.map((report) => {
        const componentData = report.component_data
          ? report.component_data.map((component) => ({
              ...component,
              total_amount:
                parseFloat(component.sold_amount) ||
                parseFloat(component.updated_quantity) *
                  parseFloat(component.rate),
            }))
          : [];

        const reportTotal = componentData.reduce(
          (sum, component) => sum + component.total_amount,
          0,
        );
        ...
        if (parentReport) {
          ...
          totals = data.reduce(
            (acc, item) => {
              acc.totalReports += 1;
              acc.totalAmount += parseFloat(item.total_amount || 0);
              acc.totalItems += item.component_data
        ...
        selectedColumns.forEach((col) => {
          if (parentReport) {
            ...
            if (col === "reportId") {
              ...
              totalsRow[columnMapping[col].header] =
                totals.allocatedQuantity.toFixed(2);
            } else if (col === "updated_quantity") {
              totalsRow[columnMapping[col].header] =
                totals.updatedQuantity.toFixed(2);
            } else if (col === "rate") {
              totalsRow[columnMapping[col].header] = "-";
  ...
  const downloadMainTablePdf = (
    ...
    try {
      ...
      const headers = selectedColumns
        .map((col) => `<th>${mainTableColumnMapping[col].header}</th>`)
        .join("");
      const rows = data
        .map((item, index) => {
          const cells = selectedColumns
            .map(
              (col) =>
                `<td>${mainTableColumnMapping[col].accessor(item, index, currentPage, itemsPerPage)}</td>`,
            )
            .join("");
          return `<tr>${cells}</tr>`;
        })
  ...
  const downloadComponentTablePdf = (
    data,
    filename,
    selectedColumns,
    title,
    ...
    try {
      // Create headers and rows based on selected columns
      const headers = selectedColumns
        .map((col) => `<th>${componentColumnMapping[col].header}</th>`)
        .join("");
      const rows = data
        .map((item, index) => {
          const cells = selectedColumns
            .map(
              (col) =>
                `<td>${componentColumnMapping[col].accessor(item, index, currentPage, itemsPerPage, parentReport)}</td>`,
            )
            .join("");
          return `<tr>${cells}</tr>`;
        })
  ...
  const filterOptions = useMemo(() => {
    ...
    return {
      center_name: [
        ...
        ...[...new Set(originalYearlyData.map((item) => item.center_name))].map(
      ...
      source_of_receipt: [
        { value: "select_all", label: "सभी चुनें" },
        ...[
          ...new Set(originalYearlyData.map((item) => item.source_of_receipt)),
        ].map((name) => ({ value: name, label: name })),
      ],
      investment_name: [
        { value: "select_all", label: "सभी चुनें" },
        ...[
          ...new Set(originalYearlyData.map((item) => item.investment_name)),
        ].map((name) => ({ value: name, label: name })),
      ],
      scheme_name: [
        { value: "select_all", label: "सभी चुनें" },
        ...[...new Set(originalYearlyData.map((item) => item.scheme_name))].map(
          (name) => ({ value: name, label: name }),
        ),
      ],
      dates: [
        ...new Set(
          originalYearlyData
            ...
            .map(
              (item) => new Date(item.billing_date).toISOString().split("T")[0],
            ),
        ),
      ]
        .sort()
  ...
  const progressPercentage =
    yearlyTotal > 0 ? ((monthlyTotal / yearlyTotal) * 100).toFixed(2) : 0;

  // Calculate comparison totals
  const allocatedTotal = yearlyTotal; // allocated * rate
  const updatedTotal = yearlyData.reduce(
    (sum, item) =>
      sum +
      parseFloat(item.updated_quantity || item.allocated_quantity) *
  ...
  const handleStatusUpdate = async () => {
    try {
      ...
      setMonthlyData((prevData) =>
        prevData.map((item) =>
          item.id === reportToUpdate.id
            ? { ...item, status: updateAction }
            : item,
        ),
      );
                ...
                <Card className="mb-4 p-3">
                  ...
                  <Row>
                    ...
                    <Col xs={12} sm={6} md={3} className="mb-2">
                      <Form.Group>
                        <Form.Label className="small-fonts fw-bold">
                          {translations.selectedDates}
                        </Form.Label>
                        <Form.Control
                          type="text"
                          value={selectedDatesInput}
                          onChange={(e) =>
                            setSelectedDatesInput(e.target.value)
                          }
                          onBlur={() =>
                            setFilters((prev) => ({
                              ...prev,
                              selected_dates: selectedDatesInput,
                            }))
                          }
                ...
                <Row className="mb-4">
                  <Col md={4}>
                    <Card className="summary-card">
                      <Card.Body>
                        <Card.Title className="total-card">
                          {translations.yearlyProgress}
                        </Card.Title>
                        <Card.Text className="display-6 small-fonts">
                  ...
                  <Col md={4}>
                    <Card className="summary-card">
                      <Card.Body>
                        <Card.Title className="total-card">
                          {translations.progressPercentage}
                        </Card.Title>
                        <Card.Text className="display-6 small-fonts">
                          {progressPercentage}%
                        </Card.Text>
                    ...
                    <>
                      <div className="table-info mb-2 d-flex justify-content-between align-items-center">
                        <span className="small-fonts">
                          ...
                          {translations.to}{" "}
                          {Math.min(indexOfLastItem, monthlyData.length)}{" "}
                          {translations.of} {monthlyData.length}{" "}
                        ...
                        <table className="responsive-table small-fonts">
                          <thead>
                            <tr>
                              ...
                              {selectedColumns.includes("reportId") && (
                                <th>{translations.reportId}</th>
                              )}
                              {selectedColumns.includes("centerName") && (
                                <th>{translations.centerName}</th>
                              )}
                              {selectedColumns.includes("sourceOfReceipt") && (
                                <th>{translations.sourceOfReceipt}</th>
                              )}
                              {selectedColumns.includes("reportDate") && (
                            ...
                            {paginatedData.map((item, index) => (
                              <React.Fragment key={item.id}>
                                <tr>
                                  ...
                                  {selectedColumns.includes("reportId") && (
                                    <td data-label={translations.reportId}>
                                      {item.bill_report_id || "-"}
                                    </td>
                                  )}
                                  {selectedColumns.includes("centerName") && (
                                  ...
                                  {selectedColumns.includes(
                                    ...
                                    <td
                                      data-label={translations.sourceOfReceipt}
                                    >
                                      {item.source_of_receipt}
                                    </td>
                                  )}
                                  {selectedColumns.includes("reportDate") && (
                                    <td data-label={translations.reportDate}>
                                      {item.billing_date
                                        ? new Date(
                                            item.billing_date,
                                      ...
                                      <div className="p-3 bg-light">
                                        ...
                                        <div className="d-flex justify-content-end mb-2">
                                          ...
                                          <Button
                                            ...
                                            size="sm"
                                            onClick={() =>
                                              downloadComponentTablePdf(
                                                item.component_data,
                                                `Component_${item.bill_report_id}_${new Date().toISOString().slice(0, 10)}`,
                                                selectedComponentColumns,
                                                `${translations.investmentName} ${translations.viewDetails}`,
                                                item,
                                              )
                                            }
                                            className="me-2"
                                                      ...
                                                      (col) => {
                                                        ...
                                                        if (
                                                          ...
                                                          return (
                                                            <td key={col}>
                                                              ₹
                                                              {component.total_amount?.toFixed(
                                                                2,
                                                              ) || "0.00"}
                                                            </td>
                                                          );
                                                ...
                                                {selectedComponentColumns.map(
                                                  (col) => {
                                                    if (col === "reportId") {
                                                      ...
                                                      ); // Show "कुल" only in Report ID column
                                                    } else if (
                                                      col ===
                                                        "investment_name" ||
                                                      col === "unit" ||
                            ...
                            <tr className="table-primary fw-bold">
                              {selectedColumns.map((col) => {
                                if (col === "sno") {
                                  ...
                                  return (
                                    <td key={col}>{monthlyData.length}</td>
                                  );
                                } else if (
                                  col === "centerName" ||
                                  col === "sourceOfReceipt" ||
              ...
              <div className="confirmation-dialog-overlay">
                <div className="confirmation-dialog">
                  <div className="confirmation-dialog-content">
                    <h5>
                      {updateAction === "accepted"
                        ? translations.confirmAccept
                        : translations.confirmReject}
                    </h5>
                    <div className="confirmation-dialog-buttons">
                      <Button
                        variant={
                          updateAction === "accepted" ? "success" : "danger"
                        }
