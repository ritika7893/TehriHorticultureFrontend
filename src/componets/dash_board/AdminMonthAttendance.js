...
import { FaClipboardList, FaFileAlt, FaEye, FaDownload, FaTimes } from 'react-icons/fa';
...
const getCellStyle = (cell) => {
  ...
  return {
    backgroundColor:
      getFillColor(cell) || '#ffffff',
    color:
      normalizeColor(font.color) || '#000000',
    fontFamily:
      font.name ||
...
function ExcelReadOnlyViewer({
  ...
  useEffect(() => {
    ...
    const loadWorkbook = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Excel file load नहीं हुई (${response.status}).`
          );
        }
        ...
        if (!loadedWorkbook.worksheets.length) {
          throw new Error(
            'Workbook में कोई worksheet नहीं मिली।'
          );
        }
        ...
        console.error(
          'Attendance Excel preview error:',
          err
        );
        ...
        if (!cancelled) {
          setError(
            err?.message ||
              'Excel file preview नहीं हो पाया।'
          );
        }
      } finally {
        if (!cancelled) {
    ...
    return (
      <div style={viewerOverlayStyle}>
        <div style={viewerStateCardStyle}>
          ...
          <h4>
            Excel Report Open नहीं हो सकी
          </h4>

          <p style={{ color: '#666' }}>
            {error ||
              'Workbook उपलब्ध नहीं है।'}
          </p>
  ...
  const mergeMap =
    createMergeMap(sheet);

  const maxRow =
    Math.max(
      sheet.rowCount || 1,
      sheet.actualRowCount || 1
    );

  const maxCol =
    Math.max(
      sheet.columnCount || 1,
      sheet.actualColumnCount || 1
    );
        ...
        <div style={viewerHeaderStyle}>
          ...
          >
            ...
            <div style={{ minWidth: 0 }}>
              ...
              >
                उपस्थिति रिपोर्ट — {report?.center_name ||
