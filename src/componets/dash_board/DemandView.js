...
const DemandView = () => {
  ...
  const [showRemainingCentersModal, setShowRemainingCentersModal] =
    useState(false); // State for Remaining Centers Modal
  ...
  const [showDeleteCenterDemandModal, setShowDeleteCenterDemandModal] =
    useState(false);
  ...
  const uniqueCenters = Array.from(
    new Set(centerDemands.map((item) => item.center_name)),
  ).filter(Boolean);
  const uniqueSubInvestments = Array.from(
    new Set(centerDemands.map((item) => item?.demand?.sub_investment_name)),
  ).filter(Boolean);
  ...
  const filteredCenterDemands = centerDemands.filter((item) => {
    ...
    const matchesSubInvestment =
      selectedSubInvestments.length === 0 ||
      selectedSubInvestments.includes(item?.demand?.sub_investment_name);
    return matchesCenter && matchesSubInvestment;
  });

  // Identify centers currently displayed in the filtered table
  const filteredUniqueCenters = Array.from(
    new Set(filteredCenterDemands.map((item) => item.center_name)),
  ).filter(Boolean);
  ...
  const handleAddDemand = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          "डिमांड बनाने में त्रुटि। कृपया बाद में पुन: प्रयास करें।",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleUpdateDemand = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          "डिमांड अपडेट करने में त्रुटि। कृपया बाद में पुन: प्रयास करें।",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleDeleteDemand = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          "डिमांड हटाने में त्रुटि। कृपया बाद में पुन: प्रयास करें।",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleEditCenterDemand = async (recordId, allocatedQuantity) => {
    ...
    if (parseFloat(editingQuantity) > parseFloat(allocatedQuantity)) {
      setEditValidationError(
        `मांगी गई मात्रा , टिहरी का कुल लक्ष्य (${allocatedQuantity}) से कम होनी चाहिए`,
      );
      return;
    }
    setIsSubmitting(true);
  ...
  const handleDeleteCenterDemand = async () => {
    ...
    try {
      ...
      setError(
        err.message ||
          "सेंटर डिमांड हटाने में त्रुटि। कृपया बाद में पुन: प्रयास करें।",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleEditQuantityChange = (e, maxQuantity) => {
    ...
    if (value > maxQty) {
      setEditValidationError(
        `मांगी गई मात्रा (${value}), टिहरी का कुल लक्ष्य (${maxQty}) से अधिक नहीं हो सकती`,
      );
      return;
    }
    setEditValidationError("");
  ...
  const exportDemandsToExcel = () => {
    const exportData = filteredDemands.map((demand, index) => ({
      ...
      ", टिहरी का कुल लक्ष्य": demand.allocated_quantity,
      इकाई: demand.unit,
  ...
  const exportCenterDemandsToExcel = () => {
    ...
    exportData.push({
      ...
      "योजना का नाम": "",
      इकाई: "",
      "मांगी गई मात्रा": filteredCenterDemands
        .reduce((sum, item) => sum + parseFloat(item.demanded_quantity || 0), 0)
        .toFixed(2),
      "कृषक विक्रय दर / अनुदान दर": "",
      "कुल राशि": filteredCenterDemands
        .reduce(
          (sum, item) =>
            sum +
            parseFloat(item.demanded_quantity || 0) *
              parseFloat(item?.demand?.rate || 0),
          0,
        )
        .toFixed(2),
    });
  ...
  const exportCenterDemandsToPDF = () => {
    ...
    totalQuantityCell.textContent = filteredCenterDemands
      .reduce((sum, item) => sum + parseFloat(item.demanded_quantity || 0), 0)
      .toFixed(2);
    ...
    totalAmountCell.textContent = filteredCenterDemands
      .reduce(
        (sum, item) =>
          sum +
          parseFloat(item.demanded_quantity || 0) *
  ...
  return (
    <>
                    ...
                    {filteredDemands.length > 0 ? (
                      ...
                      >
                        <Table
                          ...
                          hover
                          responsive
                          className="mb-0 table-thead-style"
                        >
              ...
              <Card.Header className="d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex gap-2">
                  <Button
                    ...
                    onClick={exportCenterDemandsToExcel}
                  >
                    <RiFileExcel2Line /> Excel निर्यात करें
                  </Button>
                  <Button
                    ...
                    onClick={exportCenterDemandsToPDF}
                  >
                    <RiFilePdfLine /> PDF निर्यात करें
                  </Button>
                </div>
                    ...
                    >
                      ...
                      <tbody>
                        {(() => {
                          ...
                          return Object.values(
                            filteredCenterDemands.reduce((acc, item) => {
                              if (!acc[item.center_name])
                                acc[item.center_name] = [];
                                  ...
                                  {editingCenterDemandId === item.id ? (
                                    <div className="d-flex align-items-center gap-2 flex-wrap">
                                      <Form.Control
                                        ...
                                        onChange={(e) =>
                                          handleEditQuantityChange(
                                            e,
                                            item?.demand?.allocated_quantity ||
                                              0,
                                          )
                                        }
                                      ...
                                      <Button
                                        ...
                                        disabled={
                                          isSubmitting || !!editValidationError
                                        }
                                      >
                                        {isSubmitting ? (
                                          <Spinner
                                ...
                                <td>
                                  <div className="d-flex justify-content-between align-items-center">
                                    ...
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        openDeleteCenterDemandModal(item)
                                      }
                                      title="हटाएं"
                                    >
                                      <RiDeleteBinLine />
                                ...
                                <td>
                                  <div className="d-flex gap-1">
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        openViewModal(item?.demand)
                                      }
                                      title="देखें"
                                      disabled={!item?.demand}
                                    >
                                      <RiEyeLine />
                                    </Button>
                                    <Button
                        ...
                        >
                          <td colSpan={4}>कुल</td>
                          <td>
                            {filteredCenterDemands
                              .reduce(
                                (sum, item) =>
                                  sum + parseFloat(item.demanded_quantity || 0),
                                0,
                              )
                              .toFixed(2)}
                          </td>
        ...
        <Modal.Body>
          ...
          >
            <Form.Group className="mb-3">
              <Form.Label>
                उप-मद का नाम <span className="text-danger">*</span>
              </Form.Label>
              <Form.Control
                type="text"
            ...
            <Form.Group className="mb-3">
              <Form.Label>
                इकाई <span className="text-danger">*</span>
              </Form.Label>
              <Form.Control
                type="text"
                name="unit"
            ...
            <Form.Group className="mb-3">
              <Form.Label>
                , टिहरी का कुल लक्ष्य <span className="text-danger">*</span>
              </Form.Label>
              <Form.Control
                type="number"
                step="0.01"
        ...
        <Modal.Body>
          {currentDemand && (
            <div>
              <Row className="mb-3">
                <Col md={6}>
                  <strong>उप-मद का नाम:</strong>{" "}
                  {currentDemand.sub_investment_name}
                </Col>
                <Col md={6}>
              ...
              <Row className="mb-3">
                <Col md={6}>
                  <strong>, टिहरी का कुल लक्ष्य:</strong>{" "}
              ...
              <Row className="mb-3">
                <Col md={6}>
                  <strong>कृषक विक्रय दर / अनुदान दर:</strong>{" "}
                  {currentDemand.rate}
                </Col>
                <Col md={6}>
                  <strong>कुल राशि:</strong>{" "}
                  {currentDemand.amount?.toLocaleString("hi-IN") || 0}
                </Col>
              </Row>
            </div>
          )}
      ...
      <Modal
        show={showDeleteCenterDemandModal}
        onHide={() => {
          setShowDeleteCenterDemandModal(false);
          setCenterDemandToDelete(null);
        }}
      >
        ...
        <Modal.Body>
          ...
          {centerDemandToDelete && (
            <div className="border rounded p-3 bg-light">
              <p>
                <strong>सेंटर नाम:</strong> {centerDemandToDelete.center_name}
              </p>
              <p>
                <strong>उप-मद का नाम:</strong>{" "}
                {centerDemandToDelete?.demand?.sub_investment_name}
              </p>
              <p>
                <strong>मांगी गई मात्रा:</strong>{" "}
                {centerDemandToDelete.demanded_quantity}
              </p>
              <p>
                <strong>कुल राशि:</strong> ₹
                {(
                  centerDemandToDelete.demanded_quantity *
                  (centerDemandToDelete?.demand?.rate || 0)
                ).toFixed(2)}
              </p>
            </div>
          )}
        ...
        <Modal.Footer>
          ...
          <Button
            variant="danger"
            onClick={handleDeleteCenterDemand}
            disabled={isSubmitting}
          >
            {isSubmitting ? (
              <>
      ...
      <Modal
        show={showRemainingCentersModal}
        onHide={() => setShowRemainingCentersModal(false)}
        size="lg"
      >
        <Modal.Header closeButton>
          <Modal.Title>
            शेष/मेल न खाने वाले सेंटर (Remaining Centers)
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {remainingCenters.length > 0 ? (
            ...
            <Alert variant="info" className="mb-0 text-center">
              केंद्र फ़िल्टर के सभी सेंटर वर्तमान में टेबल में मौजूद हैं। (All
              centers from the filter are currently present in the table.)
            </Alert>
          )}
        </Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => setShowRemainingCentersModal(false)}
          >
            बंद करें (Close)
          </Button>
        </Modal.Footer>
