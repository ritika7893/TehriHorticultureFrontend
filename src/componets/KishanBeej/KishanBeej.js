...
const API =
  "https://mahadevaaya.com/tehrihorticulture/tehrihorticulture_backend/api/kishanbeej";
...
const DEFAULT_CONFIG = {
  meta: {
    office: "उद्यान विशेषज्ञ कार्यालय · टिहरी गढ़वाल",
    ...
    workflow: [
      {
        n: 1,
        label: "मानक टैब",
        desc: "हर किस्म के मद यहाँ जोड़ें/संपादित करें। राजसहायता व कृषक अंश का मानक स्वतः बनेगा।",
      },
      {
        n: 2,
        label: "स्टॉक टैब",
        desc: "बीज की खरीद और केन्द्रों को आवंटन यहाँ दर्ज होता है।",
      },
      {
        n: 3,
        label: "वितरण टैब",
        desc: "रोज़ का असली काम। केन्द्र, किस्म, किसान का नाम और क्षेत्रफल भरें — बाक़ी सब अपने आप।",
      },
    ...
    printFooter:
      "उद्यान विशेषज्ञ कार्यालय · टिहरी गढ़वाल · जिला कार्ययोजना {{year}} · छपाई दिनांक {{date}}",
  },
};

function fill(str, map) {
  return str.replace(/{{(\w+)}}/g, (m, k) =>
    map[k] != null ? String(map[k]) : m,
  );
}
...
export default function KishanBeej() {
  ...
  async function saveVariety() {
    ...
    const body = {
      ...
      default_rate:
        varietyForm.default_rate === "" ? 0 : Number(varietyForm.default_rate),
      is_active: !!varietyForm.is_active,
    };
    try {
      if (editingVarietyId) {
        await apiFetch(`/varieties/${editingVarietyId}/`, {
          method: "PUT",
          body: JSON.stringify(body),
        });
        setMessage(`✓ किस्म अद्यतन — ${body.name}।`);
      } else {
        await apiFetch("/varieties/", {
          method: "POST",
          body: JSON.stringify(body),
        });
  ...
  async function deleteVariety(v) {
    if (
      !window.confirm(
        `क्या "${v.name}" किस्म हटानी है? मौजूदा मानक/क्रय/आवंटन/वितरण पर असर पड़ सकता है।`,
      )
    )
      return;
    try {
  ...
  async function addAllocation() {
    ...
    try {
      ...
      setMessage(
        `✓ ${result.centre_name} को ${result.variety_name} की ${gm(result.qty_gm)} ग्राम आवंटित।`,
      );
    } catch (e) {
      setMessage(`✘ ${e.message}`);
    }
  ...
  async function addEntry() {
    ...
    if (!t || t.gmha <= 0) {
      setMessage(
        "✘ पहले मानक टैब में इस किस्म का बीज मद (राजसहायता, इकाई किग्रा0) जोड़ें।",
      );
      return;
    }
    const requiredSeedGm = area * t.gmha;
    const available =
      opening(centreId, varietyId) - issued(centreId, varietyId);
    if (requiredSeedGm > available + 0.01) {
      const centreName =
        centres.find((c) => Number(c.id) === centreId)?.name || "";
    ...
    try {
      const itemsPayload = distItems
        .filter(
          (it) => it.standard && (Number(it.qty) > 0 || Number(it.rate) > 0),
        )
        .map((it) => ({
  ...
  React.useEffect(() => {
    editingDistIdRef.current = editingDistId;
  }, [editingDistId]);
  ...
  const purchAmt = (vid) =>
    purchases
      .filter((p) => Number(p.variety) === Number(vid))
      .reduce((s, p) => s + Number(p.amount || 0), 0);
  const purchGm = (vid) =>
    purchases
      .filter((p) => Number(p.variety) === Number(vid))
      .reduce((s, p) => s + Number(p.qty_kg || 0) * 1000, 0);
  const allotGm = (vid) =>
    allocations
      .filter((a) => Number(a.variety) === Number(vid))
  ...
  const opening = (cid, vid) =>
    allocations
      .filter(
        (a) =>
          Number(a.centre) === Number(cid) && Number(a.variety) === Number(vid),
      )
      .reduce((s, a) => s + Number(a.qty_gm || 0), 0);
  const issued = (cid, vid) =>
    entries
      .filter(
        (e) =>
          Number(e.centre) === Number(cid) && Number(e.variety) === Number(vid),
      )
  ...
  const auditChecks = useMemo(() => {
    ...
    const noSign = entries.filter(
      (e) => e.sign1 !== "हाँ" || e.sign2 !== "हाँ",
    );
    ...
    const atLimit = varieties.filter(
      (v) =>
        Number(master.purchase_limit) - purchAmt(v.id) <= 0.5 &&
        purchAmt(v.id) > 0,
    );
  ...
  function printRegister(
    ...
    title = meta.printTitle || DEFAULT_CONFIG.meta.printTitle,
  ) {
    if (!list.length) {
    ...
    printHtml(
      ...
      <h2>${title}</h2>
      <p class="pm">${fill(
        meta.printFooter || DEFAULT_CONFIG.meta.printFooter,
        {
          year: meta.year,
          office: meta.office,
          date: dmy(today()),
      ...
      <div className="kishan-beej-app">
        <header>
          <div className="in">
            ...
            <h1>{meta.title}</h1>
            <p>
              {fill(meta.subtitle, { year: meta.year, scheme: meta.scheme })}
            </p>
          </div>
        </header>
  ...
  return (
    <div className="kishan-beej-app">
      ...
      {message && (
        <div className="kishan-beej-main">
          <div className={`note ${message.startsWith("✓") ? "ok" : "bad"}`}>
            {message}
          </div>
        </div>
      )}
      ...
      <div className="kishan-beej-main">
          ...
          <section className="panel on">
            ...
            <div className="kpis">
              ...
              <Kpi
                ...
                sub={`प्राप्त ${n(
                  ledgerRows.reduce((s, r) => s + r.opening, 0),
                  0,
                )} ग्राम में से`}
                state={
                  ledgerRows.reduce((s, r) => s + r.balance, 0) < 0
                    ? "bad"
                    : "ok"
                }
              />
              <Kpi
                ...
                sub={`${ledgerRows.filter((r) => r.balance < 0).length} अधिक-वितरण, ${
                  entries.filter((e) => e.sign1 !== "हाँ" || e.sign2 !== "हाँ")
                    .length
                } हस्ताक्षर बाकी`}
        ...
        {tab === "entry" && (
          <section className="panel on">
            ...
            <p className="kishan-beej-sub">
              केवल <b>केन्द्र, किस्म, किसान का नाम और क्षेत्रफल</b> भरना है —
              बीज की मात्रा व राशि अपने आप निकल आएँगी।
            </p>
            <div className="kishan-beej-card noprint">
              <h3>
                {editingDistId
                  ? `✎ संपादन — क्रमांक ${editingDistId}`
                  : "नई प्रविष्टि"}
                {editingDistId && (
                  <button
                    className="kishan-beej-btn b2 sm"
              ...
              {editingDistId && (
                <div className="note ok" style={{ marginBottom: 10 }}>
                  संपादन मोड — सभी फ़ील्ड बदल सकते हैं। मद तालिका में नई
                  मात्रा/दर भरकर
                  <b> "मद अद्यतन करें"</b> पर क्लिक करें। किस्म बदलने पर मद
                  तालिका उस किस्म के मानक से अपने आप भर जाएगी।
                </div>
              )}
              <div className="grid g4">
                ...
                <Field label="किस्म" required>
                  ...
                  >
                    <option value="">
                      —{form.centre ? "किस्म चुनें" : "पहले केन्द्र चुनें"}—
                    </option>
                    {(form.centre
                      ? centreVarieties(Number(form.centre))
              ...
              <div className="grid g4" style={{ marginTop: 10 }}>
                ...
                <Field label="पिता/पति का नाम">
                  <input
                    value={form.father}
                    onChange={(e) => setFormValue("father", e.target.value)}
                  />
                </Field>
                <Field label="ग्राम">
                  <input
                    value={form.village}
                    onChange={(e) => setFormValue("village", e.target.value)}
                  />
                </Field>
              ...
              <div className="grid g3" style={{ marginTop: 10 }}>
                <Field label="कृषक हस्ताक्षर/अंगूठा">
                  <select
                    value={form.sign1}
                    onChange={(e) => setFormValue("sign1", e.target.value)}
                  >
                    <option>नहीं</option>
                    <option>हाँ</option>
                  </select>
                ...
                <Field label="वितरक हस्ताक्षर">
                  <select
                    value={form.sign2}
                    onChange={(e) => setFormValue("sign2", e.target.value)}
                  >
                    <option>नहीं</option>
                    <option>हाँ</option>
                  </select>
              ...
              <EntryPreview
                calc={calcEntry}
                centre={
                  centres.find((c) => Number(c.id) === Number(form.centre))
                    ?.name
                }
                stock={
                  form.centre && form.variety
              ...
              <div
                className="kishan-beej-card"
                style={{ marginTop: 12, padding: 12, background: "#fafcfa" }}
              >
                <h4 style={{ margin: "0 0 8px" }}>
                  किस्म के मानक मद{" "}
                  {currentStandardsLoading ? " (लोड हो रहे…)" : ""}
                </h4>
                {form.variety && distItems.length > 0 && (
                  ...
                  >
                    🔒 सभी फ़ील्ड किस्म के मानक से स्वतः भरे गए हैं — बदले नहीं
                    जा सकते। मानक बदलने हेतु <b>मानक</b> टैब में जाएँ।
                  </div>
                )}
                {!form.variety && (
                  <div className="empty" style={{ padding: 16 }}>
                    पहले ऊपर से किस्म चुनें — चुनते ही यहाँ मानक मद तालिका आ
                    जाएगी।
                  </div>
                )}
                {form.variety &&
                  !currentStandardsLoading &&
                  distItems.length === 0 && (
                    <div className="empty" style={{ padding: 16 }}>
                      इस किस्म का कोई मानक मद नहीं। पहले <b>मानक</b> टैब में
                      जोड़ें।
                    </div>
                  )}
                {distItems.length > 0 && (
                ...
                {editingDistId ? (
                  <>
                    <button
                      ...
                      onClick={updateDistributionItems}
                    >
                      ✓ मद अद्यतन करें
                    </button>
                    <button
            ...
            <div className="kishan-beej-card noprint">
              <div className="grid g3">
                ...
                <Field label="किस्म से छाँटें">
                  <select
                    ...
                    onChange={(e) =>
                      setFilters((p) => ({ ...p, variety: e.target.value }))
                    }
                  >
              ...
              <div className="kishan-beej-row end" style={{ marginTop: 12 }}>
                <span className="tag n">
                  {filteredEntries.length} प्रविष्टियाँ
                </span>
            ...
            <div className="kishan-beej-card table-card">
              {filteredEntries.length ? (
                ...
                <div className="empty">
                  अभी कोई प्रविष्टि नहीं। ऊपर फ़ॉर्म से पहला वितरण दर्ज करें।
                </div>
              )}
            </div>
          </section>
        ...
        {tab === "stock" && (
          <section className="panel on">
            <h2>बीज स्टॉक</h2>
            <p className="kishan-beej-sub">
              खरीद → केन्द्रों को आवंटन → केन्द्र का शेष। तीनों एक ही जगह; नीचे
              बटन से भाग बदलें।
            </p>
            <div className="seg">
              {[
                ["ledger", "केन्द्रवार शेष"],
                ["allot", "केन्द्र आवंटन"],
                ["purchase", "बीज क्रय"],
              ].map(([id, label]) => (
                <button
                  key={id}
                  className={stockTab === id ? "on" : ""}
                    ...
                    <table>
                      ...
                      <tbody>
                        {ledgerRows.map((row, index) => (
                          <tr key={`${row.centreId}-${row.varietyId}`}>
                            ...
                            <td>{gm(row.issued)}</td>
                            <td className={row.balance < 0 ? "neg" : ""}>
                              {gm(row.balance)}
                            </td>
                            <td>
                              {row.balance < 0 ? (
                      ...
                      <tfoot>
                        <tr>
                          ...
                          <td>
                            {gm(ledgerRows.reduce((s, r) => s + r.issued, 0))}
                          </td>
                          <td>
                            {gm(ledgerRows.reduce((s, r) => s + r.balance, 0))}
                          </td>
                          <td colSpan="2"></td>
                        </tr>
                      </tfoot>
              ...
              <>
                <div className="kishan-beej-card noprint">
                  ...
                  <div className="grid g4">
                    <Field label="दिनांक" required>
                      <input
                        ...
                        onChange={(e) =>
                          setAllotForm((p) => ({ ...p, date: e.target.value }))
                        }
                      />
                    </Field>
                    ...
                    <Field label="मात्रा (ग्राम)" required>
                      <input
                        ...
                        value={allotForm.qty}
                        onChange={(e) =>
                          setAllotForm((p) => ({ ...p, qty: e.target.value }))
                        }
                      />
                    </Field>
                  </div>
                  <div className="grid g3" style={{ marginTop: 10 }}>
                    <Field label="क्षेत्रफल — स्वतः (है0)">
                      <input
                        readOnly
                        value={allotCalc ? `${n(allotCalc.area, 4)} है0` : ""}
                      />
                    </Field>
                    <Field label="परियोजना लागत — स्वतः (₹)">
                      <input
                        readOnly
                        value={allotCalc ? money(allotCalc.project) : ""}
                      />
                    </Field>
                    <Field label="स्रोत/देयक सं0">
                      <input
                        value={allotForm.source}
                        onChange={(e) =>
                          setAllotForm((p) => ({
                            ...p,
                  ...
                  {allotCalc && (
                    <div
                      className={`note ${
                        allotCalc.gmv >
                        centralLeft(Number(allotForm.variety)) + 0.01
                          ? "bad"
                          : "ok"
                      }`}
                      style={{ marginTop: 12 }}
                ...
                <div className="kishan-beej-card noprint">
                  <div className="kishan-beej-row end">
                    <select
                      value={allotFilter}
                      onChange={(e) => setAllotFilter(e.target.value)}
                      style={{ maxWidth: 280 }}
                    >
                      <option value="">सभी केन्द्र</option>
                      {centres.map((c) => (
                        ...
                        downloadCsv(
                          ...
                          allocations
                            .filter(
                              (a) =>
                                !allotFilter ||
                                Number(a.centre) === Number(allotFilter),
                            )
                            .map((a, i) => [
                              i + 1,
                              dmy(a.date),
            ...
            {stockTab === "purchase" && (
              <>
                <div className="kishan-beej-card noprint">
                  ...
                  <div className="grid g3" style={{ marginTop: 10 }}>
                    <Field label="कुल राशि — स्वतः (₹)">
                      <input
                        readOnly
                        value={purchaseCalc ? money(purchaseCalc.total) : ""}
                      />
                    </Field>
                  ...
                  <div
                    className={`note ${purchaseCalc && purchaseCalc.total > purchaseCalc.left + 0.01 ? "bad" : "ok"}`}
                    style={{ marginTop: 12 }}
                  >
                    {purchaseCalc
                ...
                <div className="kishan-beej-card noprint">
                  <div className="kishan-beej-row end">
                    <span className="tag n">
                      {purchases.length} क्रय प्रविष्टियाँ
                    </span>
                    <button
                      className="kishan-beej-btn b2 sm"
                      onClick={() =>
                      ...
                      <tbody>
                        {purchases.map((p, i) => (
                          <tr key={p.id}>
                            ...
                            <td
                              className={
                                Number(p.remaining_capacity) <= 0.5 ? "neg" : ""
                              }
                            >
        ...
        {tab === "manak" && (
          <section className="panel on">
            ...
            <p className="kishan-beej-sub">
              एक बार जाँच लें, फिर छूने की ज़रूरत नहीं। यहाँ किया बदलाव केवल{" "}
              <b>आगे</b> जोड़ी जाने वाली प्रविष्टियों पर लागू होगा।
            </p>
            ...
            <div className="kishan-beej-card">
                ...
                {[
                  ...
                  <Field key={key} label={label}>
                    <input
                      ...
                      onChange={(e) =>
                        setMasterDraft((p) => ({ ...p, [key]: e.target.value }))
                      }
                    />
                  </Field>
                ))}
              ...
              <div className="kishan-beej-row" style={{ marginTop: 12 }}>
                <button className="kishan-beej-btn b1" onClick={saveMaster}>
                  सुरक्षित करें
                </button>
                <button className="kishan-beej-btn b2" onClick={resetMaster}>
            ...
            <div className="kishan-beej-card">
              ...
              <p className="kishan-beej-sub" style={{ marginBottom: 10 }}>
                जिस किस्म को खोलना हो उस पर टैप करें। मात्रा या दर बदलते ही राशि
                और जाँच अपने आप बदल जाएगी।
              </p>

              {varieties.length === 0 && (
                <div className="empty">
                  अभी कोई किस्म नहीं। नीचे से पहली किस्म जोड़ें।
                </div>
              )}
              ...
              {varieties.map((v) => {
                ...
                const okTotal =
                  Math.abs(t.total - Number(master.project_cost)) < 0.5;
                const okSubsidy =
                  Math.abs(t.subsidy - Number(master.max_subsidy)) < 0.5;
                return (
                  <details
                    key={v.id}
                  ...
                  >
                    <summary>
                      ...
                      {t.gmha > 0 && (
                        <span className="tag n">{gm(t.gmha)} ग्राम/है0</span>
                      )}

                      <span className="tag n">{money(t.total)}/है0</span>
                    </summary>
                    ...
                    <div className="dbody">
                        ...
                        editingItem?.id === item.id ? (
                          ...
                          >
                            ...
                            <div className="grid g4">
                              ...
                              <Field label="इकाई">
                                <input
                                  value={editingItem.form.unit}
                                  onChange={(e) =>
                                    setEditingItem((p) => ({
                                      ...p,
                                      form: { ...p.form, unit: e.target.value },
                                    }))
                                  }
                                />
                              </Field>
                              <Field label="मात्रा">
                                <input
                                  type="number"
                                  step="0.0001"
                                  value={editingItem.form.qty}
                                  onChange={(e) =>
                                    setEditingItem((p) => ({
                                      ...p,
                                      form: { ...p.form, qty: e.target.value },
                                    }))
                                  }
                                />
                              </Field>
                              <Field label="दर (₹)">
                                <input
                                  type="number"
                                  step="0.01"
                                  value={editingItem.form.rate}
                                  onChange={(e) =>
                                    setEditingItem((p) => ({
                                      ...p,
                                      form: { ...p.form, rate: e.target.value },
                                    }))
                                  }
                                />
                              </Field>
                          ...
                          >
                            ...
                            <div style={{ minWidth: 200 }}>
                              {n(item.qty)} × {money(item.rate)} ={" "}
                              <b>
                                {money(Number(item.qty) * Number(item.rate))}
                              </b>
                            </div>
                            <div className="kishan-beej-row">
                              <button
                              ...
                              <button
                                className="kishan-beej-btn b3 sm"
                                onClick={() =>
                                  deleteStandardItem(item.id, v.id)
                                }
                              >
                                हटाएँ
                              </button>
                      ...
                      {items.length > 0 && (
                        <div
                          className={`note ${okTotal && okSubsidy ? "ok" : "bad"}`}
                          style={{ marginTop: 10 }}
                        >
                          कुल {money(t.total)} = राजसहायता {money(t.subsidy)} +
                          कृषक अंश {money(t.farmer)}
            ...
            <div className="kishan-beej-card">
              ...
              <p className="kishan-beej-sub" style={{ marginBottom: 10 }}>
                नई किस्म जोड़ें, मौजूदा का नाम/दर/जाति बदलें या हटाएँ। किस्म
                जोड़ते ही मानक टैब में उसकी प्रविष्टि तैयार करनी होगी।
              </p>
              <div className="grid g4">
                <Field label="किस्म का नाम" required>
                  <input
                    value={varietyForm.name}
                    placeholder="जैसे — बंदगोभी Bajwa60"
                    onChange={(e) =>
                      setVarietyForm((p) => ({ ...p, name: e.target.value }))
                    }
                  />
                </Field>
                <Field label="जाति">
                  <select
                    value={varietyForm.jati}
                    onChange={(e) =>
                      setVarietyForm((p) => ({ ...p, jati: e.target.value }))
                    }
                  >
                ...
                <Field label="मूल दर (₹/किग्रा0)">
                  <input
                    ...
                    step="0.01"
                    min="0"
                    value={varietyForm.default_rate}
                    onChange={(e) =>
                      setVarietyForm((p) => ({
                        ...p,
                ...
                <Field label="स्थिति">
                  <select
                    value={varietyForm.is_active ? "1" : "0"}
                    onChange={(e) =>
                      setVarietyForm((p) => ({
                        ...p,
                  ...
                  <tbody>
                    {varieties.map((v, i) => (
                      <tr key={v.id}>
                        ...
                        <td>{v.jati || "—"}</td>
                        <td>
                          {v.default_rate != null ? money(v.default_rate) : "—"}
                        </td>
                        <td>
                          {v.is_active === false ? (
                    ...
                    {!varieties.length && (
                      <tr>
                        <td colSpan="6" className="empty">
                          अभी कोई किस्म नहीं। ऊपर से पहली किस्म जोड़ें।
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        {tab === "report" && (
          <section className="panel on">
            ...
            <p className="kishan-beej-sub">
              किस्मवार पूरी तस्वीर और सत्यापन सूची। छपी प्रति पर हस्ताक्षर
              करवाकर फाइल में रखना ही भौतिक साक्ष्य है।
            </p>
            <div className="kishan-beej-card noprint">
              <div className="kishan-beej-row">
                <button
                ...
                <table>
                  ...
                  <tbody>
                    {reportRows.map((row) => (
                      <tr key={row.v.id}>
                        ...
                        <td className={row.remaining <= 0.5 ? "neg" : ""}>
                          {money(row.remaining)}
                        </td>
                        <td>{gm(row.purchaseGm)}</td>
                        <td>{gm(row.allocated)}</td>
                        <td className={row.central < 0 ? "neg" : ""}>
                          {gm(row.central)}
                        </td>
                  ...
                  <tfoot>
                    <tr>
                      <td>कुल योग</td>
                      <td>
                        {money(reportRows.reduce((s, r) => s + r.purchase, 0))}
                      </td>
                      <td>
                        {money(reportRows.reduce((s, r) => s + r.remaining, 0))}
                      </td>
                      <td>
                        {gm(reportRows.reduce((s, r) => s + r.purchaseGm, 0))}
                      </td>
                      <td>
                        {gm(reportRows.reduce((s, r) => s + r.allocated, 0))}
                      </td>
                      <td>
                        {gm(reportRows.reduce((s, r) => s + r.central, 0))}
                      </td>
                      <td>
                        {gm(
                          entries.reduce(
                            (s, e) => s + Number(e.seed_gm || 0),
                            0,
                          ),
                        )}
                      </td>
                      <td>
                        {n(
                          reportRows.reduce((s, r) => s + r.area, 0),
                          4,
                        )}
                      </td>
                      <td>
                        {money(reportRows.reduce((s, r) => s + r.project, 0))}
                      </td>
            ...
            <div className="kishan-beej-card">
              ...
              <ul className="chk">
                {auditChecks.map((check, i) => (
                  <li key={i}>
                    <span className={`i ${check.ok ? "ok" : "bad"}`}>
                      {check.ok ? "✔" : "✘"}
                    </span>
                    <span>{check.t}</span>
                  </li>
