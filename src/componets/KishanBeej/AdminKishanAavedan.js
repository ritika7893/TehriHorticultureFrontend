...
function AdminPrintableApplication({ application }) {
  ...
  const details = [
    ["कृषक का नाम", personal.name],
    ["लिंग", personal.gender],
    ["पिता / पति का नाम", personal.father],
    ["उद्यान कार्ड संख्या", personal.udyan_card],
    ["ग्राम", personal.village],
    ["पोस्ट", personal.post],
    ["विकासखंड (ब्लॉक)", personal.block],
    ["जनपद", personal.district],
    ["मोबाइल नम्बर", personal.mobile],
    ["आधार संख्या", personal.aadhaar],
    ["कृषक की श्रेणी", personal.category],
  ].filter(([, v]) => v !== null && v !== undefined && v !== "");

  const planRows = [
    ["कुल भूमि", plan.total_land],
    ["प्रस्तावित क्षेत्रफल", plan.proposed_area],
    ["अक्षांश (Latitude)", plan.latitude],
    ["देशांतर (Longitude)", plan.longitude],
    ["भूमि पर सिंचाई सुविधा", plan.irrigation],
    ["सिंचाई स्रोत", plan.irr_source],
    ["अन्य सिंचाई स्रोत", plan.irr_other],
    ["ऊँचाई (मीटर)", plan.altitude],
    ["मुख्य सड़क से दूरी", plan.road_dist],
    ["भूमि की ढाल", plan.slope],
    ["मृदा प्रकार", plan.soil],
    ["योजना का नाम", plan.plan_scheme || scheme.name],
    ["प्रति हेक्टेयर योजना लागत", plan.cost_per_ha],
    ["आवेदन का प्रकार", plan.plan_type],
    ["समूह का नाम", plan.group_name],
    ["अन्य योजना से सहायता", plan.contribution],
    ["अन्य योजना का नाम", plan.other_scheme],
    ["बैंक का नाम", plan.bank_name],
    ["शाखा", plan.branch],
    ["बैंक खाता संख्या", plan.account],
    ["IFSC कोड", plan.ifsc],
  ].filter(([, v]) => v !== null && v !== undefined && v !== "");
    ...
    <div className="print-document print-document-preview">
      <div className="print-page">
        <div className="print-top">
          <div className="print-app-no">
            आवेदन क्रमांक: {adminPrintText(application.form_id)}
          </div>
          <div className="print-photo">
            {personal.photo ? (
              <img src={personal.photo} alt="कृषक" />
            ) : (
              <>
                फोटो
                <br />
            ...
            <table className="print-table">
              <tbody>
                ...
                {type === "kiwi" && (
                  <>
                    <tr>
                      <td className="print-key">कुल पौधे</td>
                      <td>{Math.round(208 * area).toLocaleString("en-IN")}</td>
                    </tr>
                    <tr>
                      <td className="print-key">मादा : नर</td>
                      <td>9 : 1</td>
                    </tr>
                  </>
                )}
                {type === "dragon" && (
                  <>
                    <tr>
                      <td className="print-key">RCC पिलर</td>
                      <td>{Math.round(666 * area).toLocaleString("en-IN")}</td>
                    </tr>
                    <tr>
                      <td className="print-key">कुल पौधे</td>
                      <td>{Math.round(2667 * area).toLocaleString("en-IN")}</td>
                    </tr>
                  </>
                )}
              </tbody>
        ...
        {totalCost > 0 && subsidy != null && (
          <section className="print-section print-avoid-break">
            ...
            <table className="print-table">
              <tbody>
                <tr>
                  ...
                  <td>₹{Math.round(costPerHa).toLocaleString("en-IN")}</td>
                </tr>
                <tr>
                  <td className="print-key">प्रस्तावित क्षेत्रफल</td>
                  <td>{area.toFixed(3)} हेक्टेयर</td>
                </tr>
                <tr>
                  <td className="print-key">कुल लागत</td>
                  <td>₹{Math.round(totalCost).toLocaleString("en-IN")}</td>
                </tr>
                <tr>
                  <td className="print-key">राजसहायता ({subsidy}%)</td>
                  <td>
                    ₹
                    {Math.round((totalCost * subsidy) / 100).toLocaleString(
                      "en-IN",
                    )}
                  </td>
                </tr>
                <tr>
                  <td className="print-key">कृषक अंश ({farmerShare}%)</td>
                  <td>
                    ₹
                    {Math.round((totalCost * farmerShare) / 100).toLocaleString(
                      "en-IN",
                    )}
                  </td>
                </tr>
            ...
            <p className="print-note">
              राजसहायता का निर्धारण प्रति हेक्टेयर लागत पर आनुपातिक रूप से किया
              जाएगा। स्वीकृत मानक से अधिक व्यय कृषक द्वारा स्वयं वहन किया जाएगा।
            </p>
          </section>
        )}
        ...
        <section className="print-section print-avoid-break">
          ...
          <ul className="print-list">
            {scheme.standards.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
          <p className="print-note">
            {docs.technical_standard_accepted
        ...
        <section className="print-section print-avoid-break">
          ...
          <ol className="print-list">
            {scheme.docs.map((doc) => (
              <li key={doc}>
                {Array.isArray(docs.documents) && docs.documents.includes(doc)
                  ? "☑"
                  : "☐"}{" "}
                {doc}
              </li>
        ...
        <section className="print-section print-avoid-break">
          <h2>घोषणा</h2>
          <p>
            मैं प्रमाणित करता/करती हूँ कि उपरोक्त दी गई सभी जानकारी मेरी जानकारी
            में पूर्णतः सही है।
          </p>
          <p>
            मैं प्रमाणित करता/करती हूँ कि कार्य के दौरान उपरोक्त तकनीकी मानकों
            का पालन अनिवार्यतः करूँगा/करूँगी।
          </p>
          <p className="print-check">
            {docs.declaration_accepted ? "☑" : "☐"} घोषणा स्वीकार की गई।
          </p>
        </section>

        <div className="print-signatures">
          <div>
            दिनांक: {adminPrintText(docs.application_date)}
            <br />
            स्थान: {adminPrintText(docs.place)}
          </div>
  ...
  const processCategoryData = (categoryData, type) => {
    ...
    return Array.from(formIds).map((formId) => {
      ...
      const isComplete = Boolean(
        personal.name && plan.total_land && docs.declaration_accepted === true,
      );
      ...
      return {
        ...
        created_at:
          personal.created_at || plan.created_at || docs.created_at || "",
        isComplete,
          ...
          >
            <div className="preview-modal-head">
              <div className="preview-heading-copy">
                ...
                <h2>
                  {viewingApplication.scheme ||
                    getSchemeDisplayName(viewingApplication.schemeType)}
                </h2>
